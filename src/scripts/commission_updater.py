# import time
# from libs import files, logs, skinbaron
# import logging
# import pandas as pd
# import numpy as np
# import sys
...
# logs.init_logging("commission_updater")

# refresh_active_offers = False

# # browser tools login to skinbaron -> LoginCheck
# AUTH_ID = "507a35bd76c63ad028d1d9a5751274d9f5deb9add06efffc8b9c3be73ea0f56d"
# LOGIN_TIME = "1744842907411"
# U = "3488507"
# # CHANGE THIS! CHANGE THIS! CHANGE THIS!

# # every offer higher than this commission gets updated
# commision_to_adapt_threshold = 0.08

# promotion_code = "EGG"

# # False for testing purposes
# cancel_offers_and_list = True
# # True for testing purposes
# just_list = False
# should_list = True

# if refresh_active_offers:
#     # get active offers
#     active_offers_df = skinbaron.get_active_offers_full_detail(
#         auth_id=AUTH_ID, login_time=LOGIN_TIME, u=U)
#     # log active offers
#     logging.info("active_offers_df: \n%s", active_offers_df.to_string())

#     # drop unwanted columns
#     active_offers_df = active_offers_df.drop(columns=["stackable",
#                                                         "formattedState",
#                                                         "offerLink",
#                                                         "rarityClassName",
#                                                         "isPrivate",
...
#     # cache active offers
#     active_offers_df.to_csv("./generated_files/cache/active_offers.csv", index=False)
# else:
#     active_offers_df = files.df_from_csv(csv_path="./generated_files/cache/active_offers.csv")

# active_offers_df = active_offers_df.sort_values(by=["commissionFactor", "formattedDateCreated"], ascending=False)
# logging.info("active_offers_df: \n%s", active_offers_df.to_string())

# offers_to_cancel_df = active_offers_df.loc[active_offers_df["commissionFactor"] > commision_to_adapt_threshold]
# logging.info("offers_to_cancel_df: \n%s", offers_to_cancel_df.to_string())

# offer_infos_df = offers_to_cancel_df[["name", "price", "amount"]]
# logging.info("offer_infos_df:\n%s", offer_infos_df.to_string())
# offer_infos_df.to_csv("./generated_files/cache/offer_infos.csv", index=False)

# if cancel_offers_and_list:
...
#             else:
#                 offers_to_cancel.append({"metaOfferId": row["metaOfferId"], "amount": row["amount"], "state": row["state"], "price": row["price"]})
#         logging.info("offers_to_cancel: \n%s", offers_to_cancel)

#         skinbaron.cancel_offer(auth_id=AUTH_ID, login_time=LOGIN_TIME, u=U, offers_to_cancel=offers_to_cancel)

#         time.sleep(30)
        
#     inventory_df = pd.DataFrame(skinbaron.get_inventory(
#     auth_id=AUTH_ID, login_time=LOGIN_TIME, u=U))
#     logging.debug("inventory_df: \n%s", inventory_df.to_string())

#     inventory_df = inventory_df.drop(columns=["imageUrl", "recommendedPrice",
#                                             "minimumPrice",
#                                             "maximumPrice",
#                                             "steamMarketPrice",
#                                             "isStackable",
...
#     for i, row in offer_infos_df.iterrows():
#         name = row["name"]
...
#         if should_list:
#             response = skinbaron.list_items(items=chunk, auth_id=AUTH_ID,
#                                 login_time=LOGIN_TIME, u=U, promotion_code=promotion_code)
